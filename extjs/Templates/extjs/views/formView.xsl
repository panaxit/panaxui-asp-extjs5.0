	    ...
	    formulas: {
			...
			, <xsl:value-of select="translate(@fieldName, $uppercase, $smallcase)"/>: {
				...
				<xsl:otherwise>
					, get: function(record) {
						var val = record &amp;&amp; record.isModel ? record.get('<xsl:value-of select="translate(@fieldName, $uppercase, $smallcase)"/>') : null;
						return val ? record.<xsl:value-of select="translate(@fieldName, $uppercase, $smallcase)"/> : null;
					}
					, set: function(value) {
						var record = this.get('panax_record');
						record.<xsl:value-of select="translate(@fieldName, $uppercase, $smallcase)"/> = value;
						record.set('<xsl:value-of select="translate(@fieldName, $uppercase, $smallcase)"/>', value.getId());
					}
				</xsl:otherwise>
				</xsl:choose>
			}
