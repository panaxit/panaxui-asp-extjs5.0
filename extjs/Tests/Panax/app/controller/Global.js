Ext.define('KitchenSink.controller.Global', {
    ...
    handleRoute: function(id) {
        var me = this,
            ...
            text = node.get('text'),
			catalogName = node.get('catalogName'),
			controlType = node.get('controlType'),
			mode = node.get('mode'),
        ...
        if (node.isLeaf()) {
			...
			var alias='widget.' + id
            className = ("Demo.es."+catalogName+"."+mode+"."+controlType || Ext.ClassManager.getNameByAlias(alias));
			...
			if (!ViewClass) {
				if (className) {
					...
					Ext.log.warn('Class with alias "'+alias+(className?'" or name "'+className:"")+'" not found.');
					return;
				}
			}
