Ext.define('KitchenSink.controller.Main', {
    ...
    config: {
        ...
        routes  : {
            ...
            ':mode/:table': {
                action: 'handleCatalogRoute',
                before: 'beforeHandleCatalogRoute'
			}
        }/**/
    },

    beforeHandleCatalogRoute: function(catalog, mode, action) {
		var me = this,
            bool;
        alert('before redirecting to ['+catalog+'] mode '+mode);
		
        if (!catalog) {
            bool = true;
            Ext.Msg.alert(
                'Route Failure',
                'The view for ['+catalog+'] could not be found. You will be taken to the application\'s start',
                function() {
    ...
    handleCatalogRoute: function(mode, catalog) {
        var me = this;
		alert('redirecting to ['+catalog+'] mode '+mode);
    ...
    handleRoute: function(id) {
        ...
        if (xtype) { // only leaf nodes have ids
			...
			className = (className || Ext.ClassManager.getNameByAlias(alias));
            ViewClass = Ext.ClassManager.get(className);
			if (!className) {
				Ext.log.warn('Class with alias "'+alias+'" or name "'+className+'" not found.');
				return;
			}
			if (!ViewClass) {
				Ext.create(className,{themeInfo:{}});
				ViewClass = Ext.ClassManager.get(className);
			}
